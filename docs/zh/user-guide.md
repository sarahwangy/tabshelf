...
2. 管理页面会提示"还没有连接任何文件"，点击**"连接一个文件"**来打开你之前保存过的 JSON 文件（这里弹出的是系统的"打开文件"对话框，会直接读取该文件里已有的链接，不会误建一个空文件）。如果你还没有旧文件，点击**"新建一个文件"**，按上面同样的步骤选择/新建文件即可。
...
最少查看视图下，或"最近 7 天"侧边栏筛选生效时，都不会显示 A-Z 索引；索引也会根据当前的搜索关键词过滤——如果你输入了搜索内容，只有匹配结果中存在的字母才可以点击。
